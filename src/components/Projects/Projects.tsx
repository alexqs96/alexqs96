...
export default function Projects() {
  return (
    <section>
      <h1 className="text-3xl md:text-4xl font-bold pb-1 border-b-6 border-main w-fit mb-2">
        {
          Children.toArray(
					("Projects").split('').map((e, index) => (
						<span className="fadeIn" style={{animationDelay: (index * 0.12)+'s'}}>{e}</span>
					))
          )
				}
      </h1>
      ...
      {data.map((e, index) => (
        <div key={e.name} className="grid grid-cols-1 lg:grid-cols-2 gap-10 fadeDown" style={{ animationDelay: `${(index * .55)}s` }}>
          <div className="relative w-full">
            <img src={"/img/projects/"+e.desktop} alt={e.name+" Desktop Preview"} className="aspect-video object-cover drop-shadow-xl" width={720} height={356} />

            <img src={"/img/projects/"+e.mobile} alt={e.name+" Mobile Preview"} className="absolute right-0 -bottom-1 object-cover max-w-[20%]" width={160} height={320} />
          </div>
